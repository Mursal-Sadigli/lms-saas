...
// CourseCard imported for potential use if needed later
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const enrollData = await fetchMyEnrollments(token).catch(() => mockEnrollments)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 transition-colors">
      ...
      {/* Mənim Kurslarım Header */}
      <div className="flex items-center gap-2 mb-6 text-xl font-bold text-gray-900 dark:text-white border-b border-gray-200 dark:border-slate-800 pb-4">
        <BookOpen size={24} className="text-violet-600" /> İlk Alınan Kurslar
      </div>
